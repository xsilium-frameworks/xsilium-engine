...
void DotSceneLoader::processSkyx(rapidxml::xml_node<>* XMLNode)
{
	...
	if(pElement)
	{
		//mSkyX->getVCloudsManager()->setHeight(getAttribReal(XMLNode, ""));
		//mSkyX->getVCloudsManager()->setWindSpeed();

		SkyX::VClouds::VClouds * vclouds = mSkyX->getVCloudsManager()->getVClouds() ;

		vclouds->setNoiseScale(getAttribReal(XMLNode, "noiseScale"));

		vclouds->setWindDirection(Ogre::StringConverter::parseAngle(getAttrib(XMLNode, "windDirection")));
