...
void GraphicsEntite::initEntite(Ogre::SceneManager* sceneMgr, Ogre::String nom,
        Ogre::String fileMesh) {
...
void GraphicsEntite::update(double timeSinceLastFrame) {
    ...
    if (direction != Ogre::Vector3::ZERO) {
        ...
        Ogre::Real yawAtSpeed = yawToGoal / Ogre::Math::Abs(yawToGoal) * timeSinceLastFrame
                * turnSpeed;

        // turn as much as we can, but not more than we need to
        if (yawToGoal < 0)
            yawToGoal = std::min < Ogre::Real
                    > (0, std::max < Ogre::Real > (yawToGoal, yawAtSpeed)); //yawToGoal = Math::Clamp<Real>(yawToGoal, yawAtSpeed, 0);
        else if (yawToGoal > 0)
            yawToGoal = std::max < Ogre::Real
                    > (0, std::min < Ogre::Real > (yawToGoal, yawAtSpeed)); //yawToGoal = Math::Clamp<Real>(yawToGoal, 0, yawAtSpeed);

        mMainNode->yaw(Ogre::Degree(yawToGoal));

        physicsEntite->setWalkDirection(direction.x * runSpeed * timeSinceLastFrame,
                direction.y * runSpeed * timeSinceLastFrame,
                direction.z * runSpeed * timeSinceLastFrame);
...
void GraphicsEntite::updateAnimation(double timeSinceLastFrame) {
    if (graphicsAnimation->getNomAnimationBasActuel().compare(animNames[ANIM_JUMP_START]) == 0
            && graphicsAnimation->getTime()
                    >= mBodyEnt->getAnimationState(graphicsAnimation->getNomAnimationBasActuel())->getLength()) {
        // takeoff animation finished, so time to leave the ground!
        graphicsAnimation->setAnimationBas(animNames[ANIM_JUMP_LOOP].c_str(), 1);
    } else if (graphicsAnimation->getNomAnimationBasActuel().compare(animNames[ANIM_JUMP_END])
            == 0) {
        if (graphicsAnimation->getTime()
                >= mBodyEnt->getAnimationState(graphicsAnimation->getNomAnimationBasActuel())->getLength()) {
            if (mMoving) {
                graphicsAnimation->setAnimationBas(animNames[ANIM_RUN_BASE].c_str());
                graphicsAnimation->setAnimationHaut(animNames[ANIM_RUN_TOP].c_str());
    ...
    if (graphicsAnimation->getNomAnimationHautActuel().compare(animNames[ANIM_RUN_BASE]) == 0) {
        if (graphicsAnimation->getTime()
                >= mBodyEnt->getAnimationState(graphicsAnimation->getNomAnimationHautActuel())->getLength()
                        / 2
                && graphicsAnimation->getTime() - timeSinceLastFrame
                        < mBodyEnt->getAnimationState(
                                graphicsAnimation->getNomAnimationHautActuel())->getLength() / 2) {
        ...
        if (graphicsAnimation->getTime()
                > mBodyEnt->getAnimationState(graphicsAnimation->getNomAnimationHautActuel())->getLength()) {
            if (graphicsAnimation->getNomAnimationBasActuel().compare(animNames[ANIM_IDLE_BASE])
                    == 0) {
                graphicsAnimation->setAnimationHaut(animNames[ANIM_IDLE_TOP].c_str());
            } else {
                graphicsAnimation->setAnimationBas(animNames[ANIM_RUN_BASE].c_str());
                graphicsAnimation->setAnimationHaut(animNames[ANIM_RUN_TOP].c_str());
                mBodyEnt->getAnimationState(animNames[ANIM_RUN_TOP].c_str())->setTimePosition(
                        mBodyEnt->getAnimationState(graphicsAnimation->getNomAnimationBasActuel())->getTimePosition());
            }
        }
    }
}
...
void GraphicsEntite::processEvent(Event * event) {
    if (event->hasProperty("NewDirection")) {
        deplaceEntite(
                Ogre::Vector3(atoi(event->getProperty("NewPositionX").c_str()),
    ...
    if (event->hasProperty("NewOrientation")) {
        setOrientation(
                Ogre::Quaternion(atoi(event->getProperty("NewOrientationW").c_str()),
