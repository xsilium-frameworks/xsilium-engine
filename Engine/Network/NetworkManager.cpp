...
int NetworkManager::connexionToHost(std::string url, int port) {
    ...
    if (enet_host_service(client, &eventClient, 1000) > 0
            && eventClient.type == ENET_EVENT_TYPE_CONNECT) {
...
void* NetworkManager::threadConnexion(void* arguments) {
    ...
    while ((enet_host_service(networkManager->client, &networkManager->eventClient, 10) >= 0)
        ...
        case ENET_EVENT_TYPE_RECEIVE: {
            ...
            std::istringstream archive_stream(
                    std::string((char*) networkManager->eventClient.packet->data));
