...
Patcher::Patcher() {
	...
	mCurrentVersion = Version(std::string(APP_VERSION));
	fstream res;
	path resPath = path(resolvePaths->getBinPath() + "/" + std::string(RESOURCE));
	if (!exists(resPath)) {
	...
	printf("Application version: %s \n",mCurrentVersion.Value.c_str());
	mPatchScriptPath = path(resolvePaths->getTempPath() + "/" + std::string("/patch.txt")).string();

	mProcessors.insert(std::make_pair(P_CREATE, &Patcher::processCreate));
	mProcessors.insert(std::make_pair(P_DELETE, &Patcher::processDelete));
	mProcessors.insert(std::make_pair(P_MODIFY, &Patcher::processModify));
	mProcessors.insert(std::make_pair(P_RENAME, &Patcher::processRename));
	mProcessors.insert(std::make_pair(P_ARCHIVE, &Patcher::processArchive));
...
void Patcher::validate() {
	...
	try {
		Downloader::getInstance()->_fetchPatchScript(mPatchScriptPath);
	} catch (BadPatchURL& e) {
		printf("could not retrieve patch list, aborting validation. Cause: %s \n ",e.what());
	...
	if (!mPatchScript.is_open() || !mPatchScript.good()) {
		printf("could not read patch list! \n");
...
void Patcher::buildRepositories() {
	...
	while ( !located && mPatchScript.good() )
	{
		...
		// and special paths
		if (op != P_ARCHIVE) {
			path lRoot = path(resolvePaths->getRootPath());
			elements[1] =
		...
		case P_DELETE:
			...
			break;
		case P_RENAME:
			elements[2] = path(resolvePaths->getRootPath() + "/" + elements[2]).string(); // we assume the path is relative to the root
...
bool Patcher::preprocess(Repository* inRepo) {
	std::vector<PatchStructure*> entries = inRepo->getEntries();
	bool success = true;
	std::vector<PatchStructure*>::const_iterator entry;
...
void Patcher::patch() {
	...
	for (repo = mRepos.begin(); repo != mRepos.end(); ++repo) {
		...
		Downloader::getInstance()->_fetchRepository(*repo);
		...
		std::vector<PatchStructure*> entries = (*repo)->getEntries();
		std::vector<PatchStructure*>::const_iterator entry;
...
void Patcher::processArchive(PatchStructure* inEntry, bool fCommit) {
	...
	if (tar_extract_all(t, (char*)resolvePaths->getTempPath().c_str()) != 0)
	{
	...
	for (i = 0; i < 2; ++i) {
		...
		std::vector<PatchStructure*> lEntries = inEntry->Repo->getEntries(op);
		PatchStructure* lEntry = 0;
		while (!lEntries.empty()) {
			...
			// build up target URL
			newPath =
					(path(resolvePaths->getTempPath()) / path(lEntry->Remote)).make_preferred().string();
...
void Patcher::processCreate(PatchStructure* inEntry, bool fCommit) {
	using boost::filesystem::exists;
...
void Patcher::processDelete(PatchStructure* inEntry, bool fCommit) {
	using boost::filesystem::exists;
	using boost::filesystem::is_directory;
	using boost::filesystem::create_directory;
	using boost::filesystem::path;
...
void Patcher::processModify(PatchStructure* inEntry, bool fCommit) {
	using boost::filesystem::exists;
	using boost::filesystem::path;
	using boost::filesystem::rename;
	using boost::filesystem::copy_file;
...
void Patcher::processRename(PatchStructure* inEntry, bool fCommit) {
	using boost::filesystem::exists;
	using boost::filesystem::is_directory;
	using boost::filesystem::create_directory;
	using boost::filesystem::path;
...
void Patcher::updateVersion(Version const& inVersion) {
	...
	fstream res;
	std::string resPath = path(resolvePaths->getBinPath() + "/" + std::string(RESOURCE)).string();
	res.open(resPath.c_str(), fstream::out);
	if (!res.is_open() || !res.good()) {
